...
const toolRunRowSource = span("private renderToolRuns", "private toolRunModelTitle", "tool run rows");
const baseCapabilitySource = span("private baseCapabilityPrompt", "private skillRoutePolicyPrompt", "base capability block");
const compactRouteSource = span("private compactActionRouteIndexPrompt", "private actionRouteIndexPrompt", "compact route index");
const capabilityGuideSource = span("function cancipCapabilityGuideMarkdown", "function pluginCompatibilityGuideMarkdown", "capability guide");
...
const checks = [
  ...
  ["the review baseline snapshot overlaps its file reads instead of running them serially", source.includes("const AI_MUTATION_PRIME_CONCURRENCY = 12;") && source.includes("await Promise.all(Array.from({ length: poolSize }, () => primeOne()));")],
  ["a hidden window skips the mutation-settle grace instead of waiting it out", source.includes("const capturedBeforeWait = initial.operations.size > 0 || initial.structure.length > 0;") && source.includes('if (!capturedBeforeWait && activeDocument.visibilityState === "hidden") return;')],
  ["the bridge binds from load while filesystem and network startup work stays deferred", source.includes("this.startQueueBridge();") && source.includes("void this.startAgentBridge().catch") && !source.includes("agentBridgeStartTimer") && source.includes("const cancelAgentStartup = scheduleIdleWork") && source.includes("const cancelUiEnhancements = scheduleIdleWork") && !source.includes("this.schedulePersonalizationRefresh(0);\n    this.registerInterval") && !/const cancelAgentStartup = scheduleIdleWork\([\s\S]{0,300}?startAgentBridge/.test(source)],
  ...
  ["completed process details default folded and Plan button stays on its numbered step", source.includes('this.wireDetails(step, `process-step:${stepFoldKey}`, needsIntervention, false, true)') && source.includes("processStepPlanReference") && source.includes("obcc-process-step-plan-button") && source.includes('text: `#${index + 1}`') && !source.includes('cls: "obcc-process-record-meta-button is-plan"')],
  ...
  ["process hierarchy shows total timer, folds tool payload rows by default, and omits boilerplate result rows", source.includes("obcc-process-record-timer") && source.includes("processRecordElapsedMs(items, liveProcessRecord)") && source.includes('this.wireDetails(sectionEl, `process-field:${processFoldKey}:${section.stateSuffix}`, true') && source.includes('this.wireDetails(row, `tool-run:${message.id}:${run.id}`, run.status === "pending", false, true)') && source.includes('const rows: Array<[string, string, string]> = [\n      [chinese ? \"下一步\"')],
  ...
  ["model probe keeps latency inline beside its source while the notice owns the full reply", source.includes('obcc-command-detail obcc-model-menu-detail-line') && source.includes('cls: "obcc-model-menu-source"') && source.includes('cls: "obcc-model-test-latency"') && source.includes('if (previousProbe?.ok) testLatencyEl.setText(`${previousProbe.latencyMs} ms`)') && source.includes('testLatencyEl.setText(result.ok ? `${result.latencyMs} ms` : "")') && source.includes('const reply = result.responseText?.trim() ?? ""') && source.includes('replyEl.textContent = reply') && source.includes('max_output_tokens: 96') && source.includes('max_tokens: 96') && styles.includes(".obcc-model-menu-detail-line") && styles.includes(".obcc-model-test-latency") && styles.includes(".obcc-model-probe-notice-reply") && styles.includes("white-space: pre-wrap")],
  ...
  ["task steps stay folded to level 2 while approval steps still self-open", source.includes("const needsIntervention = stepRuns.some") && source.includes("needsIntervention, false, true") && !source.includes("isLiveStep || needsIntervention") && source.includes("this.wireDetails(step, `process-step:${this.processRecordFoldKey()}:live-placeholder`, false, false, true)")],
  ...
  ["workbench registers NoteDraw after the preview host exists", !renderWorkbenchPreviewSource.includes("this.registerNoteDrawWorkbenchSurface(stage);") && renderWorkbenchPreviewSource.includes("this.mountNativeNoteDrawSurface(stage);") && workbenchSource.includes("private noteDrawPublicApi()") && workbenchSource.includes("registerSurface") && workbenchSource.includes("noteDrawWorkbenchViewport") && workbenchSource.includes("scheduleNoteDrawWorkbenchSurfaceSync") && workbenchSource.includes("registeredSurface.toggle") && workbenchSource.includes('handle.on?.("surface-changed"') && workbenchSource.includes("nativeNoteDrawSurfaceUnsubscribe") && !workbenchSource.includes("await Promise.resolve(handle.refresh?.())")],
  ...
  ["top edit-reading control follows the native Markdown action without duplicating the More menu", workbenchSource.includes('const modeIcon = sourceMode ? "eye" : "edit-3"') && workbenchSource.includes('const currentModeLabel = sourceMode ? this.plugin.t("documentEdit") : this.plugin.t("documentReading")') && workbenchSource.includes('this.workbenchTopModeButton.toggleClass("is-active", sourceMode)') && !documentMoreMenuSource.includes('this.plugin.t("documentSourceMarkdown")')],
  ...
  ["default model add action stays in Common while the default model list stays in Models", (() => {
    ...
    return !common.includes('"defaultModelOptions"')
      ...
      && defaultUi.includes('this.plugin.t("settingsDefaultModel")')
      && !defaultUi.includes('this.plugin.t("addDefaultModels")')
      && addDefaultUi.includes("promptDefaultModelPicker")
      && modelUi.includes("this.displayModelAdvancedSettings(parent);")
      && modelAdvancedUi.includes("this.plugin.settings.defaultModelOptions")
      && modelAdvancedUi.includes('groupId === "defaults"')
      && modelAdvancedUi.includes('setIcon(addDefault, "plus")');
  })()],
  ...
  ["workbench NoteDraw keeps toolbar tools usable and exposes selection while editing", workbenchSource.includes("let previousActive = this.noteDrawSurfaceIsActive(handle)") && workbenchSource.includes("const becameActive = active && !previousActive") && workbenchSource.includes("const isSelectButton = (button: HTMLButtonElement)") && workbenchSource.includes("mouse-pointer-2") && workbenchSource.includes("stage.hasClass(\"is-edit-md-mode\")") && workbenchSource.includes("hostWindow.setTimeout(() =>") && workbenchSource.includes("if (!this.noteDrawSurfaceIsActive(handle)) return;")],
  ["workbench NoteDraw guard does not hide the document root", styles.includes(".view-content.obcc-document-workbench.notedraw-body-control") && styles.includes("display: block !important")],
  ["Office workbench source opens an editable Markdown draft", workbenchSource.includes('["docx", "xlsx", "pptx", "epub"].includes(this.snapshot?.kind ?? "")') && workbenchSource.includes('const path = await this.plugin.exportDocumentConversion(snapshot.file, this.editBuffer, "md")') && workbenchSource.includes('return snapshot.rawSourceEditable')],
  ...
  ["final answer stays clean: no programmatic verification section below it", source.includes("Verification lines are no longer appended below the final answer") && source.includes('return stripStructuredChoices(stripProgrammaticRunStats(content).content).trim() || content;') && !source.includes("sections.push(`验证结果：${verificationLines[0]}`)")],
  ["state-change gate ignores interrogative prompts (打开了个什么 is a question, not a directive)", (() => { const fn = functionSource("hasExplicitExecutionDirective"); return fn.includes("questionFollowUp") && fn.includes("[了个过一]{0,2}") && /\)\s*return false;/.test(fn); })()],
  ...
  ["the per-turn payload no longer pre-computes a workspace snapshot and lets the model query live state instead", !source.includes("const workspaceState = this.buildWorkspaceStateContext();") && !/private buildWorkspaceStateContext\s*\(/.test(source) && !source.includes("WORKSPACE_STATE_MAX_TABS") && source.includes("The workspace snapshot is NOT injected any more")],
  ["every turn learns that live state is one real tool call away through the constant tool block", source.includes("includeDetailedToolProtocol: false") && source.includes("sections.push(toolPrompt);") && compactRouteSource.includes("禁止回答“看不到”或用记忆里的旧状态替答") && compactRouteSource.includes("Never reply that you cannot see it") && compactRouteSource.includes("obsidian.tabs") && compactRouteSource.includes("cancip.tools.index") && !source.includes("sections.push(this.baseCapabilityPrompt());")],
  ["the base capability block carries the action title rule, because it never goes through toolPrompt", baseCapabilitySource.includes('const titleRule = this.t("actionTitleRule");') && (baseCapabilitySource.match(/titleRule/g) ?? []).length >= 3 && baseCapabilitySource.includes('.filter(Boolean).join("\\n")')],
  ["the base capability block keeps the focused tab and the most recently edited file apart", baseCapabilitySource.includes("active 是焦点标签，最近编辑文件另算") && baseCapabilitySource.includes("active marks the focused tab; the most recently edited file is separate") && baseCapabilitySource.includes("精确路径用 type read") && baseCapabilitySource.includes("for an exact path")],
  ["step timers are recomputed on the way down, not only by the live loop that stops when a turn settles", (() => { const sync = span("private syncHeaderSessionTimer(): void {", "private headerSessionSettledTerminal(): boolean {", "header session timer sync"); const refresh = span("private refreshProcessStepTimerDom(): void {", "private toolRunProcessExplanation", "process step timer refresh"); const refreshAt = sync.indexOf("this.refreshProcessStepTimerDom();"); const runningAt = sync.indexOf("const running = this.headerSessionTimerRunning();"); return refreshAt >= 0 && runningAt > refreshAt && refresh.includes('"[data-process-record-timer-key]"') && refresh.includes('record.hasClass("is-live-process-record") && Boolean(this.activeRequest)'); })()],
  ["the folded record total is recomputed from its own rows and only counts while that record is live", source.includes("private processRecordElapsedMs(items: Array<{ message: ChatMessage }>, running = Boolean(this.activeRequest)): number {") && source.includes("this.processRecordElapsedMs(items, liveProcessRecord)") && source.includes('"data-process-record-timer-key": processFoldKey') && source.includes('"data-process-record-message-ids": items.map((info) => info.message.id).join(",")') && source.includes("(timer.dataset.processRecordMessageIds ?? \"\")")],
  ["a folded model row never lends its wall clock to the action row it is folded into", (() => { const merge = span("private mergeProcessRecordSteps", "private isLowSignalModelProcessStep", "process step merge"); return !merge.includes("elapsedMs: Math.max(left.elapsedMs, right.elapsedMs)") && merge.includes("elapsedMs deliberately keeps `left.elapsedMs`"); })()],
  ["a folded step row states the concrete action and its outcome", source.includes("cls: `obcc-process-step-action is-${primaryRun.status}`") && source.includes("describeActionPlain(primaryRun.action)") && source.includes("this.toolRunStatusLabelForRun(primaryRun)") && source.includes("if (primaryRun && !processTitle.includes(primaryActionText))") && styles.includes(".obcc-process-step-action-text") && styles.includes(".obcc-process-step-action.is-failed")],
  ["a folded tool row previews what the run returned instead of hiding the result behind another fold", source.includes("private toolRunResultPreview(run: ToolRun): string {") && source.includes('cls: "obcc-tool-run-result-preview"') && source.includes("const resultPreview = this.toolRunResultPreview(run);") && source.includes("const isEcho = (part: string): boolean =>") && styles.includes(".obcc-tool-run-result-preview")],
  ...
  ["each tool block folds to the model-written title of its action", toolRunRowSource.includes("this.toolRunModelTitle(run)") && toolRunRowSource.includes('wrap.createEl("details"') && toolRunRowSource.includes('row.createEl("summary"') && toolRunRowSource.includes("const fallbackLabel") && source.includes("title: actionModelTitle(action) || undefined") && source.includes("function modelActionTitle(")],
  ["a tool block stays folded unless it needs approval", toolRunRowSource.includes("this.wireDetails(row, `tool-run:${message.id}:${run.id}`, run.status === \"pending\", false, true)") && !toolRunRowSource.includes("`tool-run:${message.id}:${run.id}`, true,")],
  ["the folded process summary carries the turn token total", source.includes('cls: "obcc-process-record-tokens"') && source.includes("const recordTokens = steps.reduce(")],
  ["the model is told to title every action in every locale", source.includes("Every action must carry a short model-written") && source.includes("每个动作必须带一个模型自己写的短标题") && source.includes('{\\"title\\":\\"Keep the plan visible\\",\\"type\\":\\"todo\\"') && source.includes('{\\"title\\":\\"让计划可见\\",\\"type\\":\\"todo\\"')],
  ["the title rule reaches the surgical file prompts and the lightweight catalogue too, not only the full protocol", source.includes('const actionTitleRule = this.t("actionTitleRule");') && source.includes("const routedWithTitleRule = actionTitleRule && !routedToolPrompt.includes(actionTitleRule)") && source.includes("${routedWithTitleRule}") && source.includes('actionTitleRule: "Every action must carry a short model-written') && source.includes('actionTitleRule: "每个动作必须带一个模型自己写的短标题')],
  ["an action that nests its fields under args is still parsed as the declared action", (() => { const fn = functionSource("parseCancipAction"); const lift = fn.indexOf("if (isRecord(input.args)) {\n    // Only command actions carry args"); const pathGuard = fn.indexOf('if (typeof input.path !== "string")'); return lift >= 0 && fn.includes("input = { ...input.args, ...input };") && fn.includes("// Re-narrow: assigning to an `unknown` parameter drops the earlier guard") && pathGuard > lift; })()],
  ["the manual is refreshed by the deferred startup artifacts, not only by its command", source.includes("this.ensurePluginCompatibilityArtifacts(),\n          this.ensureCancipNavigationFiles(),") && source.includes("this.ensureCapabilityGuideArtifacts()\n        ])")],
  ["the manual keeps the panel and the bridge apart", capabilityGuideSource.includes("面板对话本身不走桥") && capabilityGuideSource.includes("A panel conversation never goes through the bridge") && capabilityGuideSource.includes("## 八、出问题时") && capabilityGuideSource.includes("## Troubleshooting")],
  ["the capability manual is version-owned, carries the real capability map, and is reachable from a command and the navigation index", capabilityGuideSource.includes("CANCIP_CAPABILITY_GUIDE_MARKER") && capabilityGuideSource.includes("## 三、能力清单") && capabilityGuideSource.includes("## Capability map") && capabilityGuideSource.includes("cancip.searchVault") && capabilityGuideSource.includes("obsidian.ui.applyButtonRules") && capabilityGuideSource.includes("cancip.tts.") && source.includes("if (!marker) return { path, written: false };") && source.includes("if (marker[1] === version) return { path, written: false };") && source.includes('id: "open-capability-guide"') && source.includes("this.ensureCapabilityGuideArtifacts()") && source.includes("能力与教程：${memoryLink(CANCIP_CAPABILITY_GUIDE_FILE") && source.includes("const capabilityGuide = this.plugin.capabilityGuidePath();")]
];
