...
const checks = [
  ...
  ["every non-trivial turn without a tool catalogue still learns that live state is one real tool call away", /if \(this\.plugin\.settings\.commandBusEnabled\r?\n\s*&& policy\.intent !== "trivial"\r?\n\s*&& !policy\.includeToolProtocol\r?\n\s*&& !policy\.includeToolCatalog\) \{\r?\n\s*sections\.push\(this\.baseCapabilityPrompt\(\)\);/.test(source) && source.includes("private baseCapabilityPrompt(): string {") && baseCapabilitySource.includes("cancip-action") && baseCapabilitySource.includes("obsidian.tabs") && baseCapabilitySource.includes("cancip.tools.index")],
