...
const checks = [
  ...
  ["the workspace snapshot reads real leaf state and marks the active tab", source.includes("this.workspaceTabInfos({ scope: \"all\" })") && source.includes('const mark = tab.leaf === activeLeaf ? "*" : "";') && source.includes('"工作区当前状态"') && source.includes("this.app.workspace.getActiveFile()")]
];
