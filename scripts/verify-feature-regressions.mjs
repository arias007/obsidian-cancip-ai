...
const checks = [
  ...
  ["process hierarchy shows total timer, folds tool payload rows by default, and omits boilerplate result rows", source.includes("obcc-process-record-timer") && source.includes("processRecordElapsedMs(items)") && source.includes('this.wireDetails(sectionEl, `process-field:${processFoldKey}:${section.stateSuffix}`, true') && source.includes('this.wireDetails(row, `tool-run:${message.id}:${run.id}`, run.status === "pending", false, true)') && source.includes('const rows: Array<[string, string, string]> = [\n      [chinese ? \"下一步\"')],
