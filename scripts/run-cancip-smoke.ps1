...
'@
    $editorTreeCode = @'
(async()=>{const p=app.plugins.plugins.cancip;if(!p)throw new Error('runtime unavailable');const h=Object.create(p),path='tree-smoke.md',profile={...p.activeApiProfile(),id:'tree-smoke-profile',apiUrl:'https://smoke.invalid/v1',apiKey:'smoke',model:'smoke-model'};h.settings={...p.settings,apiProfiles:[profile],activeApiProfileId:profile.id,composerAutocompleteApiProfileId:'',composerAutocompleteEnabled:true};h.editorAutocompleteCache=new Map();h.editorAutocompleteBranchCache=new Map();h.editorAutocompleteBranchPrefetches=new Map();h.editorAutocompleteModelBusy=false;h.editorAutocompleteLastModelAt=0;h.editorAutocompleteGeneration=0;h.editorAutocompleteMemoryGeneration=0;h.editorLocalAutocompleteSuffix=()=>'';h.beginEditorAutocompleteActivity=()=>()=>{};let calls=0;const lockedBatchSizes=[];h.generateEditorAutocompleteSuffix=async(_prefix,_context,_path,_profile,_excluded,roots=[])=>{calls+=1;const selected=roots.length?roots:['第一层A\n接续','第一层B','第一层C'];if(roots.length)lockedBatchSizes.push(roots.length);return JSON.stringify({candidates:selected.map(text=>({text,next:[`${text}-A`,`${text}-B`,`${text}-C`]}))})};const first=await h.editorAutocompleteCandidates('开始','开始<CURSOR>',path,{force:true}),firstReady=await h.ensureEditorAutocompleteBranches('开始','开始<CURSOR>',path,first),firstItems=first.length+first.reduce((n,root)=>n+h.editorPrefetchedAutocompleteCandidates(`开始${root}`,path).length,0),second=h.editorPrefetchedAutocompleteCandidates(`开始${first[0]}`,path),callsAfterFirst=calls,duplicateReady=await h.ensureEditorAutocompleteBranches('开始','开始<CURSOR>',path,first),dedupedPrefetch=duplicateReady===3&&calls===callsAfterFirst,alias=h.rememberEditorAutocompleteCandidateAlias('接续',path,second),recursiveReady=await h.ensureEditorAutocompleteBranches('接续','开始第一层A\n接续<CURSOR>',path,alias),third=h.editorPrefetchedAutocompleteCandidates(`接续${alias[0]}`,path),multilineAlias=first[0]==='第一层A\n接续'&&second.length===3&&alias.join('|')===second.join('|'),recursivePrefetch=recursiveReady===3&&third.length===3,readyTwelve=firstReady===3&&firstItems===12,batchedPrefetch=lockedBatchSizes.length===1&&lockedBatchSizes[0]===3;return JSON.stringify({editorTree:multilineAlias&&recursivePrefetch&&readyTwelve&&batchedPrefetch&&dedupedPrefetch,multilineAlias,recursivePrefetch,readyTwelve,batchedPrefetch,dedupedPrefetch,calls,lockedBatchSizes})})()
'@
    $editorForceTreeCode = @'
(async()=>{const p=app.plugins.plugins.cancip;if(!p)throw new Error('runtime unavailable');const h=Object.create(p),path='force-tree-smoke.md',profile={...p.activeApiProfile(),id:'force-tree-smoke-profile',apiUrl:'https://smoke.invalid/v1',apiKey:'smoke',model:'smoke-model'};h.settings={...p.settings,apiProfiles:[profile],activeApiProfileId:profile.id,composerAutocompleteApiProfileId:'',composerAutocompleteEnabled:true};h.editorAutocompleteCache=new Map();h.editorAutocompleteBranchCache=new Map();h.editorAutocompleteBranchPrefetches=new Map();h.editorAutocompleteModelBusy=false;h.editorAutocompleteLastModelAt=0;h.editorAutocompleteGeneration=0;h.editorAutocompleteMemoryGeneration=0;h.editorLocalAutocompleteSuffix=()=>'';h.beginEditorAutocompleteActivity=()=>()=>{};let calls=0;h.generateEditorAutocompleteSuffix=async(_prefix,_context,_path,_profile,_excluded,roots=[])=>{calls+=1;const selected=roots.length?roots:['换批A','换批B','换批C'];return JSON.stringify({candidates:selected.map(text=>({text,next:[`${text}-A`,`${text}-B`,`${text}-C`]}))})};const forced=await h.editorAutocompleteCandidates('换批','换批<CURSOR>',path,{force:true,excluded:['旧A','旧B','旧C']}),forcedReady=await h.ensureEditorAutocompleteBranches('换批','换批<CURSOR>',path,forced),forcedItems=forced.length+forced.reduce((n,root)=>n+h.editorPrefetchedAutocompleteCandidates(`换批${root}`,path).length,0);return JSON.stringify({forceBatchReady:forced.length===3&&forcedReady===3&&forcedItems===12&&calls===1,forcedReady,forcedItems,calls})})()
'@
    $editorMemoryCode = @'
...
'@
    $editorPromptCode = @'
(async()=>{const p=app.plugins.plugins.cancip;if(!p)throw new Error('runtime unavailable');const old={include:p.settings.includeCoreMemory,call:p.callLightweightAutocompleteModel};const captured=[];try{p.settings.includeCoreMemory=false;p.callLightweightAutocompleteModel=async(_profile,input,system,maxTokens)=>{captured.push({input,system,maxTokens});return JSON.stringify({candidates:[{text:'补全A',next:['A1','A2','A3']},{text:'补全B',next:['B1','B2','B3']},{text:'补全C',next:['C1','C2','C3']}]})};const profile={id:'smoke',name:'smoke',apiUrl:'https://smoke.invalid/v1',apiKey:'smoke',apiMode:'compatible',model:'smoke-model'},raw=await p.generateEditorAutocompleteSuffix('继续安排复诊','今天需要继续安排复诊<CURSOR>并核对材料','常用/日记/2026-07-17.md',profile),branchRaw=await p.generateEditorAutocompleteSuffix('继续安排复诊','今天需要继续安排复诊<CURSOR>并核对材料','常用/日记/2026-07-17.md',profile,[],['补全A']),root=captured[0],branch=captured[1];return JSON.stringify({memoryInjected:root.input.includes('<CURSOR>')&&!root.input.includes('相关记忆'),modelBudget:root.maxTokens===1050&&root.input.length<2600&&root.system.includes('3+9')&&root.system.includes('三组 next')&&branch.maxTokens===420&&branch.system.includes('1 个 text')&&branch.system.includes('本次共预取 3 个')&&!branch.system.includes('3×3'),modelCalled:raw.includes('补全A')&&raw.includes('C3')&&branchRaw.includes('A3')})}finally{p.settings.includeCoreMemory=old.include;p.callLightweightAutocompleteModel=old.call}})()
'@
